...
angular.module('mfcore').factory('MFDaoNoSqlAbstract', ['MFSyncPromiseProvider', 'MFDalNoSqlProxy', 'MFMappingHelper', 'MFDaoException','$injector', 'MFUtils','MFDataModelCache',
    ...
    MFDaoNoSqlAbstract.prototype._getRecordByAttributes =  function(p_attributesName) {
        ...
        return function(p_properties, p_context, p_cascadeSet) {
            ...
            //TODO refactor to handle list of attributes
            var sqlObject = null;
            MFDalNoSqlProxy.find(p_context, this.objectStoreName, sqlObject.name, sqlObject.values[0]).then(function (records) {
                if (!angular.isUndefinedOrNullOrEmpty(records)) {
                    if (records.length > 1) {
    ...
    MFDaoNoSqlAbstract.prototype._getListRecordByAttributes =  function(p_attributesName) {
        ...
        return  function (p_properties, p_context, p_cascadeSet) {
            ...
            //TODO to refactor to handle multiple p_attributesName
            var sqlObject = MFMappingHelper.convertAttributeRightIntoLeft({name: p_attributesName, values: p_properties}, self.mapping);
            console.assert(!angular.isUndefinedOrNullOrEmpty(sqlObject) && !angular.isUndefinedOrNullOrEmpty(sqlObject.values), 'p_value should be defined ');
    ...
    MFDaoNoSqlAbstract.prototype._getNbRecordByAttributes =  function(p_attributesName) {
        ...
        return function (p_properties, p_context) {
            ...
            //TODO to refactor to handle multiple p_properties
            var sqlObject = MFMappingHelper.convertAttributeRightIntoLeft({name: p_properties, values: p_values}, self.mapping);
            console.assert(!angular.isUndefinedOrNullOrEmpty(sqlObject) && !angular.isUndefinedOrNullOrEmpty(sqlObject.values), 'p_value should be defined ');

            MFDalNoSqlProxy.find(p_context, this.objectStoreName, sqlObject.name, sqlObject.values).then(function (records) {
                    console.log('[MFDaoNoSqlAbstract._getNbRecordByAttributes] records found : ', records);
...
/*
    MFDaoNoSqlAbstract.prototype._updateListRecordByAttributes = function(p_attributesName) {
        ...
        return  function(p_value, p_context, p_cascadeSet, p_toSync) {
            ...
            console.assert(!angular.isUndefinedOrNullOrEmpty(p_value)  ,'p_value should be defined ');
            console.assert(!angular.isUndefinedOrNullOrEmpty(this.mapping),'mapping should be defined');
            console.log('[MFDaoNoSqlAbstract._updateListRecordByAttributes] will update : '+p_attributesName+' = ',p_value);

            getListRecordFunction(p_value, p_context, p_cascadeSet).then(function(entities) {
                self._updateListRecord(entities, p_context, p_cascadeSet, p_toSync).then(function() {
                    console.log('[MFDaoNoSqlAbstract._updateListRecordByAttributes]  updated : ',entities);
                    deferred.resolve();
                }, function(error) {
                    deferred.reject(error);
                });
