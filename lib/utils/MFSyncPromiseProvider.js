...
function qFactory(nextTick, exceptionHandler) {
  var $qMinErr = angular.$$minErr('$q', TypeError);
  ...
  Deferred.prototype = {
    ...
    $$resolve: function(val) {
      ...
      try {
        if ((angular.isObject(val) || angular.isFunction(val))) then = val && val.then;
      ...
      if ((this.promise.$$state.status <= 0) && callbacks && callbacks.length) {
        nextTick(function() {
          ...
          for (var i = 0, ii = callbacks.length; i < ii; i++) {
            ...
            try {
              result.notify(angular.isFunction(callback) ? callback(progress) : progress);
            } catch (e) {
              exceptionHandler(e);
            }
          }
  ...
  var handleCallback = function handleCallback(value, isResolved, callback) {
    ...
    try {
      if (angular.isFunction(callback)) callbackOutput = callback();
    } catch (e) {
