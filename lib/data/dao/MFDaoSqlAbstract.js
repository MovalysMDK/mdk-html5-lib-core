...
angular.module('mfcore').factory('MFDaoSqlAbstract', ['MFSyncPromiseProvider', 'MFDalSqlProxy', 'MFMappingHelper', 'MFDaoException', '$injector', 'MFUtils',
    function ($qSync, MFDalSqlProxy, MFMappingHelper, MFDaoException, $injector, MFUtils) {
         ...
         *
         * @description
         * Return the id to set to the Entity. If entity already have an id different than -1, then it
         * keeps its id. Otherwise we use the function getNextId the generate an new id for the entity.
         *
         * @param p_entity
         * @param p_MFContext
         * @returns {deferred.promise|{then, catch, finally}}
         */
        MFDaoSqlAbstract.prototype.getEntityId = function (p_entity, p_MFContext) {
            var deferred = $qSync.defer();
            var self = this;

            if (angular.isUndefinedOrNull(p_entity.idToString) || (p_entity.idToString) === -1) {
                self.getNextId(p_MFContext).then(
                    function (returnedSuccess_nextId) {
                        deferred.resolve(returnedSuccess_nextId);

                    },
                    function (returnedError_nextId) {
                        console.error('getEntityId(): error: ', returnedError_nextId);
                        p_MFContext.addError(returnedError_nextId);
                        deferred.reject(returnedError_nextId);
                    }
                );
            } else {
                deferred.resolve(p_entity.idToString);
            }
            return deferred.promise;
        };
        ...
        /**
         * @ngdoc method
         * @name MFDaoSqlAbstract#executeQueryToReadIfEntityExist
         * @function
         *
         * @description
         * Executes a query on the DAL for reading entities. Allows to factorize the code in generated DAOs.
         ...
         * @returns {Array<Object>} Returns an Array of one or several entities, including children if required // TODO doc fix
         */
        MFDaoSqlAbstract.prototype.executeQueryToReadIfEntityExist = function (p_callerName, p_context, p_sqlQuery, p_sqlParameters) {
            var deferred = $qSync.defer();
                ...
                function (returnedSuccess) { /* SUCCESS */
                    ...
                    try {
                        ...
                        if (returnedSuccess.rows.length === 0) { // No result
                            ...
                            result = angular.isUndefinedOrNull(aObjectsToReturn) ? false : true;
                        }
                        deferred.resolve(result);
                    }
                    catch (error) {
        ...
        MFDaoSqlAbstract.prototype.getEntitiesByProperty = function getEntitiesByProperty(p_propertyName, p_propertyValue, p_context) {
            ...
            if (!askForFullList) {
                ...
                var sqlObject = MFMappingHelper.convertAttributeRightIntoLeft({
                    name: p_propertyName,
                    values: values
                }, self.mapping);
        ...
        /**
         * @ngdoc method
         * @name MFDaoSqlAbstract#exist
         * @function
         *
         * @description return true if the entity exists in database
         *
         * @param p_entity
         * @param p_context
         * @returns {deferred.promise|{then, catch, finally}}
         */
        MFDaoSqlAbstract.prototype.exist = function exist(p_entity, p_context) {
            var self = this;
            var deferred = $qSync.defer();
            ...
            var o_sqlQueryParams;
            var p_propertyName = 'id';
            var p_propertyValue = p_entity.idToString;
            var values = MFUtils.toArray(p_propertyValue);
            ...
            this.executeQueryToReadIfEntityExist(
                'getEntitiesBy' + p_propertyName, p_context, o_sqlQuery, o_sqlQueryParams
            ).then(
                function (result_success) {
                    deferred.resolve(result_success);
        ...
        MFDaoSqlAbstract.prototype.saveEntity = function saveEntity(p_entity, p_context, p_toSync) {
            ...
            this.getEntityId(p_entity, p_context).then(function (returned_EntityId) {
                    ...
                    for (var attrIdx = 0; attrIdx < self.mapping.attributes.length; attrIdx++) {
                        ...
                        if (currAttrDef.identifier) {
                            if (angular.isUndefinedOrNull(returned_EntityId)) {
                                valueToInsert = p_entity[currAttrDef.rightAttr];
                            } else {
                                valueToInsert = returned_EntityId;
                                p_entity[currAttrDef.rightAttr] = returned_EntityId;
                            }
                        }
                        else {
                ...
                function (returnedError_EntityId) {
                    console.error('saveEntity(): error: ', returnedError_EntityId);
                    p_context.addError(returnedError_EntityId);
                    deferred.reject(returnedError_EntityId);
                }
            );
        ...
        MFDaoSqlAbstract.prototype.getChildrenIdsToRemove = function getChildrenIdsToRemove(p_context, p_entity, p_childAttributeName) {
            ...
            var childrenIdList = [];
            for (var j = 0, length = childrenList.length; j < length; j += 1) {
                if (childrenList[j] && childrenList[j].id) {
                    childrenIdList.push(childrenList[j].id);
                }
            }

            //   1.2 get the list of children IDs removed
            var sqlQuery = 'SELECT ' + childIdColumn + ' FROM ' + childTable + ' WHERE ' + childColumnPointingParent + ' = ' + p_entity.idToString + ' AND ' + childIdColumn + ' NOT IN ( ' + childrenIdList.join(', ') + ' )';
            var sqlParameters = [];
            ...
            MFDalSqlProxy.executeQuery(p_context, sqlQuery, sqlParameters).then(
                function (returnedSuccess) {
                    ...
                    if (returnedSuccess.rows.length > 0) {
                        for (var i = 0; i < returnedSuccess.rows.length; i++) {
                            idsList.push(returnedSuccess.rows.item(i)[childIdColumn]);
                        }
                    }
        ...
        MFDaoSqlAbstract.prototype.loadChildrenIfNeeded = function (p_context, parent, p_cascadeSet) {
            ...
            if (this.cascadeDefinition.length > 0 && p_cascadeSet.length > 0 && !angular.isUndefinedOrNull(parentEntity)) {
                var callsToDo = [];
                for (var childDefIdx = 0, childDefIdxLength = this.cascadeDefinition.length; childDefIdx < childDefIdxLength; childDefIdx += 1) {
                    ...
                    console.assert(!angular.isUndefinedOrNull(childDefinition.childAttrPointingParent), 'childrenDefinition[' + childDefIdx + '].childAttrPointingParent should be defined');
                    ...
                    for (var i = 0, iLength = p_cascadeSet.length; i < iLength; i += 1) {
                        if ((p_cascadeSet[i].foreignEntitiesNames[p_cascadeSet[i].key] === childDefinition.parentAttrPointingChild) && p_cascadeSet[i].entityName === this.entityName) {
                            console.log('Will process the cascade item ' + p_cascadeSet[i]);
                            if (angular.isArray(parent)) {
                                for (var iParent = 0, iParentLength = parent.length; iParent < iParentLength; iParent += 1) {
                                    this.addDaoChildCall(p_context, parent[iParent], p_cascadeSet, childDefinition, callsToDo);
                                }
                            } else {
                ...
                if (callsToDo.length > 0) {
                    ...
                    loadChildrenDeferred.resolve(parentEntity);
                }
            }
            else {
                loadChildrenDeferred.resolve(parentEntity);
            ...
            if (angular.isArray(parent)) {
                for (var resIdx = 0; resIdx < parent.length; resIdx++) {
                    if (!childDefinition.preFilled) {
                        ...
                        if (!angular.isUndefinedOrNull(parent[resIdx][childDefinition.parentAttrPointingChild]) && !angular.isUndefinedOrNull(parent[resIdx][childDefinition.parentAttrPointingChild].id)) {
                ...
                if (!childDefinition.preFilled) {
                    ...
                    if (!angular.isUndefinedOrNull(parent[childDefinition.parentAttrPointingChild].idToString)) {
                        ...
                        if (!angular.isUndefinedOrNull(children)) {
                            ...
                            if (children.length > 0) {
                                ...
                                children.forEach(function (child) {
                                    ...
                                    // Browsable from parent to child => many-to-one or one-to-one
                                ...
                                var fillParent = function (instance) {
                                    ...
                                    if (angular.isArray(parentAttrPointingChild)) {
                                        parentAttrPointingChild.length = 0;
