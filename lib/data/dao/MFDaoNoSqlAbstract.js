...
angular.module('mfcore').factory('MFDaoNoSqlAbstract', ['MFSyncPromiseProvider', 'MFDalNoSqlProxy', 'MFMappingHelper', 'MFDaoException', '$injector', 'MFUtils', 'MFNoSqlIdGenerator',
  function ($qSync, MFDalNoSqlProxy, MFMappingHelper, MFDaoException, $injector, MFUtils, MFNoSqlIdGenerator) {
    ...
    /**
     * @ngdoc method
     * @name MFDaoNoSqlAbstract#_getRecordForDeletionById
     * @function
     *
     * @description
     * Returns a Record by id with composite children.
     *
     * @param {MFContext} p_context MFContext object
     * @param {Array<String>} p_cascadeSet Array containing children tables names to process
     * @param {Integer} p_Id Integer id of the Record to get
     * @returns {Record} Returns the Record, including children if required
     */
    MFDaoNoSqlAbstract.prototype._getRecordForDeletionById = function _getRecordForDeletionById(p_id, p_context, p_cascadeSet) {
      var deferred = $qSync.defer();
      var self = this;
      console.assert(!angular.isUndefinedOrNullOrEmpty(this.objectStoreName), 'objectStoreName should be defined');
      console.assert(!angular.isUndefinedOrNullOrEmpty(p_id), 'p_id should be defined');
      ...
      MFDalNoSqlProxy.find(p_context, this.objectStoreName, p_id).then(function (record) {
        if (record !== null) {
          ...
          var entity = self.record2Entity(p_context, record);
          self.resolveCascade(entity, p_context, p_cascadeSet, 'readToDelete').then(function () {
            console.log('[MFDaoNoSqlAbstract._getRecordForDeletionById] 1 record found : ', entity);
            deferred.resolve(entity);
          }, function (error) {
            deferred.reject(error);
          });
    ...
    MFDaoNoSqlAbstract.prototype._saveRecord = function (p_entity, p_context, p_cascadeSet, p_toSync) {
      ...
      MFNoSqlIdGenerator.initIdGeneratorIfNeeded(p_context, this.objectStoreName).then(function () {

        // Update the ID
        var nextId = MFNoSqlIdGenerator.getNextId(self.objectStoreName);
        ...
        self.completeChildren(p_entity);
        ...
        self.resolveCascade(p_entity, p_context, p_cascadeSet, 'update', p_toSync).then(function () {
          var record = self.entity2Record(p_context, p_entity);
    ...
    MFDaoNoSqlAbstract.prototype._deleteRecordById = function (p_entityId, p_context, p_cascadeSet, p_toSync) {
      var deferred = $qSync.defer();
      var self = this;
      var i, lenCascade = this.cascadeDefinition.length;
      ...
      self._getRecordForDeletionById(p_entityId, p_context, p_cascadeSet).then(function (entity) {
    ...
    var deleteMissingChildren = function (p_entity, p_attribute, p_context, p_childDao, p_cascadesForDelete, p_toSync, p_childAttrPointingParent) {

      return p_childDao._getListRecordByAttributeBis(p_childAttrPointingParent, p_entity, p_context, p_cascadesForDelete).then(function (existingChildren) {

        if (!angular.isUndefinedOrNullOrEmpty(existingChildren)) {
          var entitiesToDelete = [];
          var i, ii;
          var rightIdAttribute = MFMappingHelper.getRightIdAttribute(p_childDao.mapping);
          existingChildren = MFUtils.toArray(existingChildren);

          if (angular.isArray(p_entity[p_attribute])) {
            var remainingChildrenIds = [];
            for (i = 0, ii = p_entity[p_attribute].length; i < ii; ++i) {
              remainingChildrenIds[i] = p_entity[p_attribute][i][rightIdAttribute];
            }
            ...
            for (i = 0, ii = existingChildren.length; i < ii; ++i) {
              var currChildId = p_entity[p_attribute][rightIdAttribute];
              if (currChildId !== existingChildren[i][rightIdAttribute]) {
                entitiesToDelete.push(existingChildren[i]);
              }
            }
          }
          return p_childDao._deleteListRecord(entitiesToDelete, p_context, p_cascadesForDelete, p_toSync);
        }
      });
    };
    ...
    var deletePreviousChild = function (entity, attribute, p_context, childDao, subCascade, p_toSync) {
      ...
      return this._getRecordForDeletionById(entity.idToString, p_context, []).then(function (oldEntity) {
    ...
    MFDaoNoSqlAbstract.prototype.resolveCascade = function (p_entity, p_context, p_cascades, p_operation, p_toSync, p_cascadesForDelete) {
      p_cascades = p_cascades || [];
      var self = this;
      var deferred = $qSync.defer();
      //p_operation = p_operation === undefined ? 'read' : p_operation;
      var operation =  (p_operation === undefined || p_operation === 'readToDelete') ? 'read' : p_operation;
      var cascadePromises = [];
      var doneCascades = [];

      // Check if we need to follow a Cascade from this Entity
      for (var i = 0; i < p_cascades.length; i++) {
        var cascade = p_cascades[i];
        if (cascade.entityName === p_entity._type) {
          // Follow the cascade
          var cascadeEndpoint = cascade.foreignEntitiesNames[cascade.key];
          cascadePromises = cascadePromises.concat(executeOperationOnCascadesEntities(self, operation, p_entity, cascadeEndpoint, p_cascades, p_cascadesForDelete, p_context, p_toSync));
          doneCascades.push(cascadeEndpoint);
        }
      }

      // Retrieve automatically for composite:true association
      for (var attribute in this.cascadeDefinition) {
        if (doneCascades.indexOf(attribute) === -1) {// Not already done
          if (this.cascadeDefinition.hasOwnProperty(attribute)) {
            var attributeDefinition = this.cascadeDefinition[attribute];
            if ( attributeDefinition.composite && ((operation === 'delete' && !angular.isUndefinedOrNull(p_entity[attribute])) || p_operation === 'readToDelete')) {
              cascadePromises = cascadePromises.concat(executeOperationOnCascadesEntities(self, operation, p_entity, attribute, p_cascades, p_cascadesForDelete, p_context, p_toSync));
              doneCascades.push(attribute);
            } else if ((attributeDefinition.cardinality === 'many-to-many' || attributeDefinition.cardinality === 'one-to-many' || attributeDefinition.cardinality === 'one-to-one')
              && operation === 'delete' && !angular.isUndefinedOrNull(attributeDefinition.childAttrPointingParent)) {
              ...
              var cascadePromise = updateChildrenFkWhenParentRemoved(p_entity, p_context, foreignDao, p_cascades, p_toSync, attributeDefinition.childAttrPointingParent);
              cascadePromises.push(cascadePromise);
            } else {
              // The cascade wasn't requested by the user and is not composite
              // So set the value to null (Because MFMappingHelper use the factory to create an incomplete object)
              p_entity[attribute] = null;
            }
          }
        }
      }
    ...
    function executeOperationOnCascadesEntities(self, p_operation, p_entity, p_attribute, p_cascade, p_cascadesForDelete, p_context, p_toSync) {
      var deferred = $qSync.defer();
      var setResult = function (entity, attribute, results) {
        entity[attribute] = results;
      };
      var setFirstResult = function (entity, attribute, results) {
        entity[attribute] = results[0];
      };

      var cascadePromises = [];
      var cascadePromise;
      var attributeDefinition = self.cascadeDefinition[p_attribute];
      console.assert(!angular.isUndefinedOrNull(attributeDefinition.foreignEntity), 'the property foreignEntity is required');

      var foreignDao = self.daoTable[attributeDefinition.foreignEntity];

      if (foreignDao === undefined) {
        self.daoTable[attributeDefinition.foreignEntity] = $injector.get(attributeDefinition.foreignEntity + 'DaoNoSql');
        foreignDao = self.daoTable[attributeDefinition.foreignEntity];
      }
      ...
      switch (p_operation) {
          ...
          switch (attributeDefinition.cardinality) {
            case 'one-to-one':
              if (attributeDefinition.relationOwner) {
                // Don't go through if relation is 0..1 and no endpoint is present.
                if (p_entity[p_attribute] !== null) {
                  cascadePromise = foreignDao._getRecordById(p_entity[p_attribute][MFMappingHelper.getRightIdAttribute(foreignDao.mapping)], p_context, p_cascade);
                  cascadePromise.then(setResult.bind(null, p_entity, p_attribute));
                  cascadePromises.push(cascadePromise);
                }
              } else {
                //the FK is owned by the foreign/child entity
                cascadePromise = foreignDao._getListRecordByAttributeBis(attributeDefinition.childAttrPointingParent, p_entity, p_context, p_cascade);
                cascadePromise.then(setFirstResult.bind(null, p_entity, p_attribute));
                cascadePromises.push(cascadePromise);
              }
              break;
            case 'many-to-one':
            ...
            case 'many-to-many':
              var ids = [];
              for (var i = 0, ii = p_entity[p_attribute].length; i < ii; ++i) {
                ids[i] = p_entity[p_attribute][i][MFMappingHelper.getRightIdAttribute(self.mapping)];
              }
              cascadePromise = foreignDao._getListRecordByIds(ids, p_context, p_cascade);
              cascadePromise.then(setResult.bind(null, p_entity, p_attribute));
              cascadePromises.push(cascadePromise);
              break;
            default:
        ...
        case 'save':
          switch (attributeDefinition.cardinality) {
            case 'one-to-one':
            case 'many-to-one':
              cascadePromise = foreignDao._saveOrUpdateRecord(p_entity[p_attribute], p_context, p_cascade, p_toSync);
              cascadePromises.push(cascadePromise);
              break;
            case 'one-to-many':
            case 'many-to-many':
              cascadePromise = foreignDao._saveOrUpdateListRecord(p_entity[p_attribute], p_context, p_cascade, p_toSync);
              cascadePromises.push(cascadePromise);
              break;
            default:
              console.error('unknown cardinality : ' + attributeDefinition.cardinality);
        ...
        case 'update':
          switch (attributeDefinition.cardinality) {
            case 'one-to-one':
              if (attributeDefinition.composite || p_cascadesForDelete && p_cascadesForDelete.indexOf(p_attribute) > -1) {
                if (attributeDefinition.relationOwner) {
                  cascadePromise = deletePreviousChild.call(self, p_entity, p_attribute, p_context, foreignDao, p_cascadesForDelete, p_toSync);
                  cascadePromises.push(cascadePromise);
                }
                else {
                  cascadePromise = deleteMissingChildren(p_entity, p_attribute, p_context, foreignDao, p_cascadesForDelete, p_toSync, attributeDefinition.childAttrPointingParent);
                  cascadePromises.push(cascadePromise);
                }
              }
              cascadePromise = foreignDao._saveOrUpdateRecord(p_entity[p_attribute], p_context, p_cascade, p_toSync);
        ...
        case 'delete':
          switch (attributeDefinition.cardinality) {
            case 'one-to-one':
              cascadePromise = foreignDao._deleteRecord(p_entity[p_attribute], p_context, p_cascade, p_toSync);
              cascadePromises.push(cascadePromise);
              break;
            case 'many-to-one':
              ...
              break;
            case 'one-to-many':
              cascadePromise = foreignDao._deleteListRecord(p_entity[p_attribute], p_context, p_cascade, p_toSync);
              cascadePromises.push(cascadePromise);
              break;
            case 'many-to-many':
          ...
          break;
        default:
          var errorMsg3 = 'Unknown operation : ' + p_operation;
          console.error(errorMsg3);
          deferred.reject(errorMsg3);
          break;
      }

      return cascadePromises;
    }
