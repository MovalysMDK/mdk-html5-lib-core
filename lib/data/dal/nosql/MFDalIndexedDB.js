...
angular.module('mfcore').factory('MFDalIndexedDB', ['MFSyncPromiseProvider', 'MFException', 'MFDalIndexedDBTransaction', 'MFConfigurationService', 'MFIntegerConverter', 'MFDalException', 'MFUtils',
    function ($qSync, MFException, MFDalIndexedDBTransaction, MFConfigurationService, MFIntegerConverter, MFDalException, MFUtils) {
        ...
        MFDalIndexedDB.openDatabase = function () {
            ...
            if (!Modernizr.indexeddb) {
                ...
                if (_dbConnection) {
                    ...
                    engineRequest.onupgradeneeded = function (event) {
                        deferred.reject('The database '+databaseConfig.name+' (v. '+databaseConfig.version+') does not exist');
                        // deferred.resolve(onUpgradeNeeded(event.target.result));
                    };
                    //callback error db
                    ...
                    engineRequest.onversionchange = function(event) {
                        console.error('another transaction still opened is creating object stores');
                        console.log(event);
                        deferred.resolve(_dbConnection);
                    };
                }
        ...
        MFDalIndexedDB.initDatabase = function (createStoresScripts) {
            var deferredInitDB = $qSync.defer();

            if (!Modernizr.indexeddb) {
                deferredInitDB.reject('Your browser doesn\'t support a stable version of IndexedDB. Such and such feature will not be available.');
            } else {

                console.info('[INIT NO SQL BDD] starting...');
                var databaseConfig = MFConfigurationService.getValue('databaseConfig');
                var engineRequest = engine.indexedDB.open(databaseConfig.name, MFIntegerConverter.fromString(databaseConfig.version));

                //callback success db
                engineRequest.onsuccess = function (event) {
                    console.warn('[INIT NO SQL BDD] '+databaseConfig.name+' database already initialized');
                    _dbConnection = event.target.result;
                    deferredInitDB.resolve(_dbConnection);
                };

                //callback error db
                engineRequest.onerror = function(event){
                    deferredInitDB.reject('[INIT NO SQL BDD] Cannot open the database '+databaseConfig.name+' -- error: ' + (event.target.webkitErrorMessage || event.target.errorCode));
                };


                engineRequest.onupgradeneeded = function (event) {
                    _dbConnection = event.target.result;
                    var nbObjectStoresToCreate = createStoresScripts.length;

                    var creationSuccessListener = function(event) {
                        nbObjectStoresToCreate--;
                        if(nbObjectStoresToCreate === 0){
                            console.info('[INIT NO SQL BDD] completed');
                            deferredInitDB.resolve(_dbConnection);
                        }
                    };

                    console.log('[INIT NO SQL BDD] will init the database '+databaseConfig.name+' ');
                    for (var j = 0; j < createStoresScripts.length; ++j) {
                        var objectStores = createStoresScripts[j];
                        nbObjectStoresToCreate = nbObjectStoresToCreate-1 + Object.keys(objectStores).length ;
                        for (var objectStoreName in objectStores) {
                            if (objectStores.hasOwnProperty(objectStoreName)) {
                                console.log('[INIT NO SQL BDD] creating objectStore "' + objectStoreName+'"');
                                var objectStore = _dbConnection.createObjectStore(objectStoreName, { 'keypath': objectStores[objectStoreName].keypath });
                                var indices = objectStores[objectStoreName].createIndex;
                                for (var k = 0; k < indices.length; ++k) {
                                    console.log('[INIT NO SQL BDD] creating index "' + indices[k].name+'"');
        ...
        MFDalIndexedDB.clearObjectStore = function (context, objectStoreName) {
            ...
            var deferred = $qSync.defer();
            var objectStore = context.dbTransaction.getObjectStore(objectStoreName,true);
            ...
            request.onerror = function (error) {
                ...
                context.dbTransaction.rollback(error);
        ...
        MFDalIndexedDB.findByFilter = function (context, objectStoreName, filteringFunction) {
            var shouldFilter = angular.isFunction(filteringFunction);          

            var deferred = $qSync.defer();
            var objectStore = context.dbTransaction.getObjectStore(objectStoreName,false);
            console.assert(!angular.isUndefinedOrNull(objectStore), 'Object store ' + objectStoreName + ' not found');
            ...
            objectStore.openCursor().onsuccess = function (event) {
                ...
                if (cursor) {
                    ...
                    if (!shouldFilter || filteringFunction(resultValue)) {
                    ...
                    if (context.rollback) {
                        context.dbTransaction.rollback('explicit rollback');
                    }
                }
            };
            ...
            objectStore.openCursor().onerror = function (errorEvent) {
                ...
                context.addError(dalException.toString());
                console.debug('call DB transaction rollback');
                context.dbTransaction.rollback(errorEvent);
                deferred.reject(dalException);
            };
        ...
        MFDalIndexedDB.findByProperties = function (context, objectStoreName, propertyValues, propertyName, p_filteringFunction) {
            ...
            var shouldFilter = angular.isFunction(p_filteringFunction);
            ...
            if (angular.isUndefinedOrNull(objStoreIndex)) {
                return MFDalIndexedDB.findByFilter(
                    ...
                    objectStoreName,
                    function (record) {
                        return propertyValues.indexOf(record[propertyName]) !== -1 && (!shouldFilter || p_filteringFunction(record));
                    }
                );
            }
            else {
                ...
                try {
                    ...
                    var onSuccess = function (deferredElement, event) {
                        var cursor = event.target.result;
                        if (cursor) {
                            if (!shouldFilter || p_filteringFunction(cursor.value)) {
                    ...
                    if (e.code === window.DOMException.NOT_FOUND_ERR) {
                        ...
                        console.error(dalException);
                        context.addError(dalException.toString());
                        context.dbTransaction.rollback(e);
                        deferred.reject(dalException);
                    }
                }
                return deferred.promise;
        ...
        MFDalIndexedDB.prepare = function (context, objectStoreNames, writable) {
            if (objectStoreNames.length > 0) {
                context.dbTransaction.resolve(objectStoreNames, writable);
            }
        };
