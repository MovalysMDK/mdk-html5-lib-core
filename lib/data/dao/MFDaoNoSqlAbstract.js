...
angular.module('mfcore').factory('MFDaoNoSqlAbstract', ['MFSyncPromiseProvider', 'MFDalNoSqlProxy', 'MFMappingHelper', 'MFDaoException', '$injector', 'MFUtils',
  function ($qSync, MFDalNoSqlProxy, MFMappingHelper, MFDaoException, $injector, MFUtils) {
    ...
    MFDaoNoSqlAbstract.prototype.resolveCascade = function (entity, p_context, cascades, operation, p_toSync, p_cascadesForDelete) {
      var self = this;
      var deferred = $qSync.defer();
      ...
      for (var i = 0; i < cascades.length; i++) {
        ...
        if (cascade.entityName === entity._type) {
          // Follow the cascade
          var cascadeEndpoint = cascade.foreignEntitiesNames[cascade.key];

          cascadePromises = cascadePromises.concat(retrieveCascadesEntities(self, operation, entity, cascadeEndpoint, cascades, p_cascadesForDelete, p_context, p_toSync));
        ...
        if (doneCascades.indexOf(attribute) === -1) {// Not already done
          if (this.cascadeDefinition.hasOwnProperty(attribute)) {
            ...
            if (attributeDefinition.composite) {
              cascadePromises = cascadePromises.concat(retrieveCascadesEntities(self, operation, entity, attribute, cascades, p_cascadesForDelete, p_context, p_toSync));
              ...
              var cascadePromise = updateChildrenFkWhenParentRemoved(entity, p_context, foreignDao, cascades, p_toSync, attributeDefinition.childAttrPointingParent);
          ...
          switch (attributeDefinition.cardinality) {
            case 'one-to-one':
              if (attributeDefinition.composite || p_cascadesForDelete && p_cascadesForDelete.indexOf(p_attribute) > -1) {
                if (attributeDefinition.relationOwner) {
                  cascadePromise = deletePreviousChild(p_entity, p_attribute, p_context, foreignDao, p_cascadesForDelete, p_toSync);
                  cascadePromises.push(cascadePromise);
                }
                else {
