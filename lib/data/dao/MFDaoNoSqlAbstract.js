    ...
    MFDaoNoSqlAbstract.prototype.resolveCascade = function (p_entity, p_context, p_cascades, p_operation, p_toSync, p_cascadesForDelete) {
      ...
      for (var i = 0; i < p_cascades.length; i++) {
        ...
        if (cascade.entityName === p_entity._type) {
          ...
          cascadePromises = cascadePromises.concat(executeOperationOnCascadesEntities(self, p_operation, p_entity, cascadeEndpoint, p_cascades, p_cascadesForDelete, p_context, p_toSync));
      ...
      for (var attribute in this.cascadeDefinition) {
        if (doneCascades.indexOf(attribute) === -1) {// Not already done
          if (this.cascadeDefinition.hasOwnProperty(attribute)) {
            ...
            if (attributeDefinition.composite) {
              cascadePromises = cascadePromises.concat(executeOperationOnCascadesEntities(self, p_operation, p_entity, attribute, p_cascades, p_cascadesForDelete, p_context, p_toSync));
              ...
              && p_operation === 'delete' && !angular.isUndefinedOrNull(attributeDefinition.childAttrPointingParent)) {
