            ...
            if (!self.failed) {
                ...
                for (var i = 0; i < self.actions.length; ++i) {
                    ...
                    if (action.status === action.Status.RUNNING || action.status === action.Status.NOT_STARTED) {
                        var errorMessage = 'previous action failed  => failure of action ' + actionContainer.action.type + '';
                        action.rejectPromise(errorMessage,actionContainer.cluster.context);
                    }
                }
