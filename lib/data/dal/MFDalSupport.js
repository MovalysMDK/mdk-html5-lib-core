...
angular.module('mfcore').factory('MFDalSupport', ['MFIntegerConverter', 'MFConfigurationService', 'MFDeviceInfo', function (MFIntegerConverter, MFConfigurationService, MFDeviceInfo) {
    ...
    MFDalSupport.prototype.evaluatePlatform = function () {
        ...
        if(MFDeviceInfo.isNative){
            this.platform = MFDeviceInfo.deviceOS;
        }
        else {
            this.platform = MFDeviceInfo.browser;
        }

        var prefBaseTypes;
        if (dataBaseType[platformType].hasOwnProperty(this.platform.toLowerCase())) {
            prefBaseTypes = dataBaseType[platformType][this.platform.toLowerCase()];
        }
        else {
            console.warn(this.platform+'not found in the configuration => use default');
            prefBaseTypes  = dataBaseType[platformType]['default'];
            console.assert(!angular.isUndefinedOrNullOrEmpty(dataBaseType[platformType]['default']),'no default configuration found');
        }
        ...
        if(prefBaseTypes){
            ...
            if(MFDeviceInfo.isNative) {
                ...
                platformVersion = MFIntegerConverter.fromVersion(MFDeviceInfo.browserVersion);
            }
    ...
    MFDalSupport.prototype.getDalSupportDeviceAndBase = function () {
        ...
        return this.platform + '_' + this.getDalSupportBase();
    };
