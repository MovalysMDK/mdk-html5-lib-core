...
angular.module('mfcore').factory('MFDaoSqlAbstract', ['MFSyncPromiseProvider', 'MFDalSqlProxy', 'MFMappingHelper', 'MFDaoException', '$injector', 'MFUtils',
    function ($qSync, MFDalSqlProxy, MFMappingHelper, MFDaoException, $injector, MFUtils) {
        ...
        MFDaoSqlAbstract.prototype.loadChildrenIfNeeded = function (p_context, parent, p_cascadeSet) {
            ...
            if (this.cascadeDefinition.length > 0 && !angular.isUndefinedOrNull(parentEntity)) {
                var callsToDo = [];
                for (var childDefIdx = 0, childDefIdxLength = this.cascadeDefinition.length; childDefIdx < childDefIdxLength; childDefIdx += 1) {
                    ...
                    //var childAlreadyDefined = !angular.isUndefinedOrNull(parentEntity[childDefinition.parentAttrPointingChild]) && (parentEntity[childDefinition.parentAttrPointingChild]._fulfilled || angular.isArray(parentEntity[childDefinition.parentAttrPointingChild]) && parentEntity[childDefinition.parentAttrPointingChild].length > 0 && parentEntity[childDefinition.parentAttrPointingChild][0]._fulfilled);
                    ...
                    if (p_cascadeSet.length > 0) {
                        for (var i = 0, iLength = p_cascadeSet.length; i < iLength; i += 1) {
                            if ((p_cascadeSet[i].foreignEntitiesNames[p_cascadeSet[i].key] === childDefinition.parentAttrPointingChild) && p_cascadeSet[i].entityName === this.entityName) {
                                ...
                                this.addDaoChildCall(p_context, parentEntity, p_cascadeSet, childDefinition, callsToDo);
                            }
                        }
                    }
                    // }
            ...
            if (angular.isArray(parent)) {
                for (var resIdx = 0; resIdx < parent.length; resIdx++) {
                    if (!childDefinition.preFilled) {
                        ...
                        if (!angular.isUndefinedOrNull(parent[resIdx][childDefinition.parentAttrPointingChild]) && !parent[resIdx][childDefinition.parentAttrPointingChild]._fulfilled && !angular.isUndefinedOrNull(parent[resIdx][childDefinition.parentAttrPointingChild].id)) {
