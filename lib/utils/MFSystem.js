	...
	MFSystem.getAsset = function(filePath, cached) {
		...
		if (typeof filePath !== 'string') {
			...
			var getFile = function getFile(filePath) {
                var result;
                if(cached && Modernizr.localstorage) {
                    result = localStorage['asset#' + filePath];
                }
				if (result === undefined) {

                    try {
                        $http.get(filePath,{ignoreLoadingBar: true}).then(
                            function(result) {
                                try {
                                    if (cached && Modernizr.localstorage) {
                                        localStorage['asset#' + filePath] = JSON.stringify(result.data);
                                    }
                                    console.log(filePath+' downloaded',result);
                                    deferred.resolve(result.data);
                                }
                                catch(error){
                                    console.error(filePath+' download failed',error);
                                    deferred.reject(error);
                                }
                            }, function (error) {
                                console.error(filePath+' download or JSON parsing failed',error);
                                deferred.reject(filePath+' download or JSON parsing failed');
                            });
                    }
                    catch(error){
                        console.log(filePath+' download failed',error);
                        deferred.reject(error);
                    }
				} else {
                    console.log(filePath+' retrieved from cache');
