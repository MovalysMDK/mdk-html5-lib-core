            ...
            initTask.runTaskNoSql = function (context, firstLaunch) {
                if (firstLaunch) {
                    MFSystem.getAsset('assets/data/nosql/create_userdata.json', false).then(function(response) {
                        MFTransactionManager.run(context, function(context, param) {
                            console.assert(angular.isUndefinedOrNull(response.data) || !angular.isArray(response.data),'create_userdata.json should be like {store1:[rec11,rec12], store2:[rec21,rec22]}');
                            ...
                            for (objectStoreName in response.data) {
                                if (response.data.hasOwnProperty(objectStoreName)) {
                                    console.assert(angular.isArray(response.data[objectStoreName]),'create_userdata.json should be like {store1:[rec11,rec12], store2:[rec21,rec22]}');
