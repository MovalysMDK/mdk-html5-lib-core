    ...
    var convertLeftIntoRight = function (leftObject, mapping, rightObjectsCache, existingRightObject) {
      ...
      for (var attrMappingIndex in mapping.attributes) {
        if (mapping.attributes.hasOwnProperty(attrMappingIndex)) {
          ...
          // 3 cases:
          // mapping.attributes[].leftAttr is an array && mapping.attributes[].multiple === true
          // mapping.attributes[].leftAttr is an array && mapping.attributes[].multiple === false
          ...
          if (angular.isArray(attrMapping.leftAttr)) {// mapping.attributes[].leftAttr is an array
            if (attrMapping.multiple) {
              ...
              var leftObjArrayForLeftAttribute = leftObject[attrMapping.leftAttr[0]];
              console.assert(angular.isArray(leftObjArrayForLeftAttribute), 'The attr "' + attrMapping.leftAttr[0] + '" of ' + leftObject.constructor.name + ' should be an array');

              for (var leftChildIdx = 0; leftChildIdx < leftObjArrayForLeftAttribute.length; leftChildIdx++) {
                // the goal here is to flatten the tree of objects of each child
                var leftChildValue = leftObjArrayForLeftAttribute[leftChildIdx];
                            
                // From previous assert : if multiple then leftAttr.length === 2.
                for (var leftSubchildDepth = 1; leftSubchildDepth < attrMapping.leftAttr.length && !angular.isUndefinedOrNull(leftSubChild); leftSubchildDepth++) {
                  leftChildValue = leftChildValue[attrMapping.leftAttr[leftSubchildDepth]];
                }
                leftValue[leftChildIdx] = leftChildValue;
              }
            } else { // attrMapping.multiple === false
          ...
          if (angular.isDefined(leftValue)) {
            // If mapping.attributes[].multiple === true, leftValue should be an array
            ...
            if (angular.isArray(attrMapping.rightAttr)) {// mapping.attributes[].rightAttr is an array
              if (attrMapping.rightAttr.length >= 2) {
                ...
                if (!attrMapping.identifier || !angular.isUndefinedOrNull(leftValue)) {
                  ...
                  } else { // attrMapping.multiple === true
                    rightObject[attrMapping.rightAttr[0]] = [];

                    //here, rightListChild is the list attribute of the main right object
                    var rightListChild = rightObject[attrMapping.rightAttr[0]];

                    if (angular.array(leftValue)) {
                      for (var childIdx = 0; childIdx < leftValue.length; childIdx++) {
                        ...
                        if (cacheActive && attrMapping.childIdentifier && leftValue[childIdx] !== -1) {
                          var subchildCacheId = MFDataModelCache.getCacheIdentifier(attrMapping.rightFactory[attrMapping.rightFactory.length - 1], leftValue[childIdx]);
                          if (!angular.isUndefinedOrNull(subchildCacheId)) {
                            rightObjectsCache[subchildCacheId] = rightSubChild;
                          }
                        }
                      }// for leftValue[]
                    } //angular.array(leftValue)
                  } // attrMapping.multiple === true
                }
