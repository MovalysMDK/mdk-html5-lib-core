...
MFDalSqlModule.factory('MFDalSqliteWindows8', ['$rootScope', '$q', '$http', 'ASYNC', 'MFDalSqlAbstract', function ($rootScope, $q, $http, ASYNC, MFDalSqlAbstract) {
            ...
            p_sqlWithParams.forEach(function (sqlItem, index) {
                if (p_mode === ASYNC.ONE) {
                    $q.when('start').then(function () {
                        // execute request
                        return p_db.oneAsync(sqlItem.sql, sqlItem.params || [])
                            .then(function (results) {
                             // if execute request success
                    ...
                    $q.when('start').then(function () {
                        // execute request
                        return p_db.runAsync(sqlItem.sql, sqlItem.params || [])
                            .then(function (results) {
                            // if execute request success
                                successResults.splice(index, 0, results);
                            });
    ...
    return angular.extend(new MFDalSqlWindows8(), {
        ...
        run: function (p_transaction) {
            ...
            $q.when(true).then(function (value) {
                ...
                //Action if load file is success
