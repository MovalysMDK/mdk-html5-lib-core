...
angular.module('mfcore').factory('MFDaoSqlAbstract', ['MFSyncPromiseProvider', 'MFDalSqlProxy', 'MFMappingHelper', 'MFDaoException','$injector','MFUtils','MFDataModelCache',
    function ($qSync, MFDalSqlProxy, MFMappingHelper, MFDaoException,$injector,MFUtils,MFDataModelCache) {
        ...
        MFDaoSqlAbstract.prototype.produceQueryInPart = function(p_ids) {
            ...
            var r_sInPart;
            for(var i=0; i<p_ids.length; i++) { // produces ?, ?, ?, ...
                if(r_sInPart === undefined) { r_sInPart  =   '?'; }
                else                        { r_sInPart += ', ?'; }
            }
            return r_sInPart;
        };
                            ...
                            for (var i = 0; i < returnedSuccess.rows.length; i++) {
                                ...
                                if (recordsGroupingEnabled) {
                                    ...
                                    for (var childrenListColumn in childrenGroupings) {
                                        ...
                                        //var currentChildId = currentRecord[childIdColumn];
        ...
        MFDaoSqlAbstract.prototype.saveEntity = function saveEntity(p_entity, p_context, p_toSync) {
            ...
            self.getNextId(p_context).then(
                ...
                function(returnedSuccess_nextId) {
                    ...
                    firstPartSqlQuery = firstPartSqlQuery.slice(0, -1) + ') '; // remove last comma and add bracket
                    secondPartSqlQuery = secondPartSqlQuery.slice(0, -1) + ');'; // remove last comma and add bracket
        ...
        MFDaoSqlAbstract.prototype.updateEntity = function updateEntity(p_entity, p_context, p_toSync) {
            ...
            if(!p_entity._fulfilled){
                ...
                firstPartSqlQuery = firstPartSqlQuery.slice(0, -1); // remove last comma
                console.assert(!angular.isUndefinedOrNullOrEmpty(wherePartSqlQuery) && !angular.isUndefinedOrNullOrEmpty(idValue), 'ID not found');
                o_sqlParameters.push(idValue);// ID is the last param of the query
        ...
        MFDaoSqlAbstract.prototype.executeQueryToWrite = function(p_callerName, p_context, p_sqlQuery, p_sqlParameters, p_toSync) {
            ...
            MFDalSqlProxy.executeQuery(p_context, p_sqlQuery, p_sqlParameters).then(
                function (returnedSuccess) { /* SUCCESS */
                    ...
                    var queryType = p_sqlQuery.substring(0, 6).toLowerCase(); // must be equal to: insert, update or delete
                    var resultKey;
                    var resultValue;
                    var affectedId;
                    ...
                    if ((that.syncDisabled === false) && (p_toSync === true)) {
...
//                    if(!loadAllChildren && p_cascadeSet.length > 0){
//                        p_context.addError('Not able to empty the cascade');
//                        loadChildrenDeferred.reject('Not able to empty the cascade');
//                    }else {
...
//                }
            ...
            if (angular.isArray(parent)) {
                for (var resIdx = 0; resIdx < parent.length; resIdx++) {
                    if (!childDefinition.preFilled) {
                        ...
                        if(!parent[resIdx][childDefinition.parentAttrPointingChild]._fulfilled){
                            daoMethodParam.push(parent[resIdx][childDefinition.parentAttrPointingChild].idToString);
                        }
                    }
                }
            }
        ...
        MFDaoSqlAbstract.prototype.addDaoChildCall = function addDaoChildCall(p_context, parent,p_cascadeSet,childDefinition, callsToDo){
            ...
            if(daoMethodParam.length > 0){
                ...
                if(!childDefinition.preFilled){
                    daoMethodToCall = childDefinition.readAction+childEntity+'By'+childDefinition.childAttrPointingParent.capitalize()+'id';
                    console.log('since the child attribute is not prefilled, get the children by their foreign key which points to the parent');
                }
                else {
                    daoMethodToCall = childDefinition.readAction+childEntity+'ById';
                }
                        ...
                        function(children){
                            ...
                            if(children.length > 0) {
                                if (angular.isArray(parent)) {
                                    ...
                                    //1. build an associative array with key=parent id / value= array of children

                                    var childrenByParent = {};
                                    for (var childIdx = 0; childIdx < children.length; childIdx++) {
                                        var childAttrPointingParent = children[childIdx][childDefinition.childAttrPointingParent];
                                        ...
                                        if (angular.isArray(childAttrPointingParent )) {
                                            for (var pointedParentIdx = 0; pointedParentIdx < childAttrPointingParent.length; pointedParentIdx++) {
                                                ...
                                                childrenByParent[childAttrPointingParent[pointedParentIdx].idToString].push(children[childIdx]);
                                            }
                                        }
                                        else {
                                            if (angular.isUndefinedOrNull(childrenByParent[children[childIdx][childDefinition.childAttrPointingParent].idToString])) {
                                                childrenByParent[children[childIdx][childDefinition.childAttrPointingParent].idToString] = [];
                                            }
                                            childrenByParent[children[childIdx][childDefinition.childAttrPointingParent].idToString].push(children[childIdx]);
                                        }
                                    }


                                    //2. fill the attribute of the parent
                                    for (var parentIdx = 0; parentIdx < parent.length; parentIdx++) {

                                        parent[parentIdx][childDefinition.parentAttrPointingChild] = childrenByParent[parent[parentIdx].idToString];
                                        /*                                        var parentAttrToSet = parent[parentIdx][childDefinition.parentAttrPointingChild];

                                         if (angular.isArray(parentAttrToSet)) {
                                         Array.prototype.push.apply(parentAttrToSet,childrenByParent[parent[parentIdx].idToString]);
                                         }
                                         else {
                                         parent[parentIdx][childDefinition.parentAttrPointingChild] = childrenByParent[parent[parentIdx].idToString][0];
                                         }
                                         */
                                    }
                                    ...
                                    // fill the attribute of the parent
                                    parent[childDefinition.parentAttrPointingChild] = children;
                                    /*
                                     if (angular.isArray(parent[childDefinition.parentAttrPointingChild])) {
                                     Array.prototype.push.apply(parent[childDefinition.parentAttrPointingChild], children);
                                     }
                                     else {
                                     parent[childDefinition.parentAttrPointingChild] = children;
                                     }
