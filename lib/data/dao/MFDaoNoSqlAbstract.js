...
angular.module('mfcore').factory('MFDaoNoSqlAbstract', ['MFSyncPromiseProvider', 'MFDalNoSqlProxy', 'MFMappingHelper', 'MFDaoException', '$injector', 'MFUtils',
    function ($qSync, MFDalNoSqlProxy, MFMappingHelper, MFDaoException, $injector, MFUtils) {
        ...
        MFDaoNoSqlAbstract.prototype.initIdGeneratorIfNeeded = function (p_context) {
            ...
            var self = this;
            if (!self.lastId) {
              MFDalNoSqlProxy.getLastId(p_context, self.objectStoreName).then(function (id) {
                if (!self.lastId) {
                  self.lastId = id;
                  deferred.resolve(id);
                } else { // Can be the case if multiple promises were launched while self.lastId was null
