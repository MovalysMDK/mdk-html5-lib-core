...
angular.module('mfcore').factory('MFDaoNoSqlAbstract', ['MFSyncPromiseProvider', 'MFDalNoSqlProxy', 'MFMappingHelper', 'MFDaoException','$injector', 'MFUtils','MFDataModelCache',
    ...
    MFDaoNoSqlAbstract.prototype._saveRecord = function(p_entity, p_context, p_cascadeSet, p_toSync) {
        ...
        this.initIdGeneratorIfNeeded(p_context).then(function() {
            ...
            record[MFMappingHelper.getLeftIdAttribute(self.mapping)] = nextId;
            ...
            self.resolveCascade(p_entity, p_context, p_cascadeSet, 'update').then(function() {
                MFDalNoSqlProxy.insert(p_context, self.objectStoreName, record).then(function(result) {
    ...
    MFDaoNoSqlAbstract.prototype._saveListRecord = function(p_entitiesList, p_context, p_cascadeSet,p_toSync) {
        ...
        this.initIdGeneratorIfNeeded(p_context).then(function() {
            ...
            for (var i = 0; i < p_entitiesList.length; ++i) {
                var nextId = self.getNextId();
                records[i][MFMappingHelper.getLeftIdAttribute(self.mapping)] = nextId;
                p_entitiesList[i][MFMappingHelper.getRightIdAttribute(self.mapping)] = nextId;
                ids[i] = nextId;
            ...
            $qSync.all(cascadePromises).then(function() {
                MFDalNoSqlProxy.insert(p_context, self.objectStoreName, records).then(function(result) {
                    if (p_toSync && !self.syncDisabled) {
    ...
    MFDaoNoSqlAbstract.prototype._updateRecord = function(p_entity, p_context, p_cascadeSet,p_toSync, p_cascadeSetForDelete) {
        ...
        this.resolveCascade(p_entity, p_context, p_cascadeSet, 'update').then(function() {
            MFDalNoSqlProxy.save(p_context, self.objectStoreName, record).then(function(result) {
                if (p_toSync && !self.syncDisabled) {
                    self.registerEntitiesForSync(p_context, p_entity[MFMappingHelper.getRightIdAttribute(self.mapping)]).then(function() {
                        deferred.resolve(result);
                    });
    ...
    MFDaoNoSqlAbstract.prototype._updateListRecord = function(p_entitiesList,p_context, p_cascadeSet,  p_toSync, p_cascadeSetForDelete) {
        ...
        $qSync.all(cascadePromises).then(function() {
            MFDalNoSqlProxy.save(p_context, self.objectStoreName, records).then(function(result) {
                var ids = new Array(p_entitiesList.length);
                for (var i = 0; i < p_entitiesList.length; ++i) {
    ...
    MFDaoNoSqlAbstract.prototype._deleteRecordById = function(p_entityId, p_context, p_cascadeSet, p_toSync) {
        ...
        this._getRecordById(p_entityId, p_context, []).then(function(result) {
    ...
    MFDaoNoSqlAbstract.prototype.registerEntitiesForSync = function(p_context, p_ids) {
        ...
        MFDalNoSqlProxy.save(p_context, 'ObjectToSynchronize', records).then(function() {
            deferred.resolve();
        }, function(error) {
            deferred.reject(error);
        });
    ...
    MFDaoNoSqlAbstract.prototype.resolveCascade = function(entity, p_context, cascade, operation, p_toSync) {
        var deferred = $qSync.defer();
        var self = this;
        var attributes = this.cascadeDefinition;
        ...
        var completeChildren = function(entity, attribute) {
            var children = Array.isArray(entity[attribute]) ? entity[attribute] : [entity[attribute]];
            for (var i = 0, ii = children.length; i < ii; ++i) {
                var mappingDefinitionRecord = self.cascadeDefinition[attribute];
                if (mappingDefinitionRecord !== undefined) {
                    var parentAttribute = children[i][mappingDefinitionRecord.rightAttribute];
                    if (!Array.isArray(parentAttribute)) {
                        children[i][mappingDefinitionRecord.rightAttribute] = entity;
                    }
                }
            }
        };
        for (var attribute in attributes) {
            var index = cascade.indexOf(attribute);
            ...
            if (index > -1) {
                ...
                case 'read':
                    ...
                    case 'many-to-one':
                        ...
                        break;
                    case 'one-to-many':
                        cascadePromise = foreignDao._getListRecordByAttributeBis(attributeDefinition.rightForeignKey, entity[MFMappingHelper.getRightIdAttribute(self.mapping)], p_context, subCascade);
                ...
                case 'update':
                    ...
                    case 'one-to-one':
                    case 'many-to-one':
                        completeChildren(entity, attribute);
                        cascadePromise = foreignDao._saveOrUpdateRecord(entity[attribute], p_context, subCascade, p_toSync);
                        cascadePromises.push(cascadePromise);
                        break;
                    case 'one-to-many':
                ...
                switch (operation) {
                case 'update':
                    if (attributeDefinition.strong) {
                        switch (attributeDefinition.cardinality) {
                        case 'one-to-one':
                        case 'many-to-one':
                            completeChildren(entity, attribute);
                            cascadePromise = foreignDao._saveOrUpdateRecord(entity[attribute], p_context, [], p_toSync);
                            cascadePromises.push(cascadePromise);
                            break;
                        case 'one-to-many':
                        case 'many-to-many':
                            cascadePromise = foreignDao._getListRecordByAttributeBis(attributeDefinition.rightForeignKey, entity[MFMappingHelper.getRightIdAttribute(self.mapping)], p_context, []);
                            cascadePromise.then(cascadeChildrenDeletion.bind(null, entity[attribute], p_context, [], p_toSync, cascadePromises));
                            cascadePromises.push(cascadePromise);
                            completeChildren(entity, attribute);
                            cascadePromise = foreignDao._saveOrUpdateListRecord(entity[attribute], p_context, [], p_toSync);
                            cascadePromises.push(cascadePromise);
                            break;
                        }
                    }
                ...
                case 'delete':
                    if (attributeDefinition.strong) {
                        ...
                        case 'one-to-one':
                        case 'many-to-one':
                            cascadePromise = foreignDao._deleteRecord(entity[attribute], p_context, [], p_toSync);
                            cascadePromises.push(cascadePromise);
                            break;
                        case 'one-to-many':
                            cascadePromise = foreignDao._deleteListRecord(entity[attribute], p_context, [], p_toSync);
                            cascadePromises.push(cascadePromise);
                            break;
                        case 'many-to-many':
                            break;
                        ...
                        case 'many-to-many':
                            cascadePromise = foreignDao._getListRecordByAttributeBis(attributeDefinition.rightForeignKey, entity[MFMappingHelper.getRightIdAttribute(self.mapping)], p_context, subCascade);
                            cascadePromise.then(cascadeChildrenUpdate.bind(null, entity[attribute], p_context, subCascade, p_toSync, cascadePromises));
                            cascadePromises.push(cascadePromise);
                            cascadePromise = foreignDao._saveOrUpdateListRecord(entity[attribute], p_context, subCascade, p_toSync);
                            cascadePromises.push(cascadePromise);
                            break;
